...
import { DollarSign, Users, Activity, Search, Plus, Settings2, ShieldQuestion, Trash2, X, AlertTriangle, Calendar, Award, Loader2, MapPin, FileText, CreditCard, Lock, RefreshCw, Copy, Check, CheckCircle, Link, Zap, Bot } from 'lucide-react';
...
const SuperAdminDashboard: React.FC = () => {
    ...
    const featuresToToggle: {key: keyof NonNullable<Client['courtesyFeatures']>, label: string, icon: React.ReactNode}[] = [
        { key: 'financialModule', label: 'Módulo Financeiro', icon: <DollarSign size={16} /> },
        { key: 'whatsappBot', label: 'Chatbot WhatsApp', icon: <Bot size={16} /> }
    ];
                  ...
                  <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-gray-800">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Telefone</label><input value={formData.settings?.phone} onChange={e => setSettingsValue('phone', maskPhone(e.target.value))} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]" maxLength={15} /></div>
                            <div className="space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">E-mail</label><input type="email" value={formData.settings?.email} onChange={e => setSettingsValue('email', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-gray-800">
                            <div className="space-y-1.5">
                                ...
                                <div className="flex items-center bg-slate-100 dark:bg-gray-800/50 rounded-xl border border-slate-200 dark:border-gray-700 p-1 h-[46px]">
                                    <button onClick={() => setSettingsValue('documentType', 'CNPJ')} className={`flex-1 h-full py-2 text-xs font-bold rounded-lg transition-all ${formData.settings?.documentType === 'CNPJ' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-500 dark:text-gray-400'}`}>CNPJ</button>
                                    <button onClick={() => setSettingsValue('documentType', 'CPF')} className={`flex-1 h-full py-2 text-xs font-bold rounded-lg transition-all ${formData.settings?.documentType === 'CPF' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-500 dark:text-gray-400'}`}>CPF</button>
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"
                        ...
                        <div className="grid grid-cols-12 gap-x-4 gap-y-4 pt-4 border-t border-slate-100 dark:border-gray-800">
                            <div className="col-span-12 md:col-span-3 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1 flex justify-between items-center">CEP {isLoadingCep && <Loader2 size={12} className="animate-spin" />}</label><input value={formData.settings?.address?.cep} onChange={e => setAddressValue('cep', maskCEP(e.target.value))} onBlur={handleCEPBlur} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]" maxLength={9} /></div>
                            <div className="col-span-12 md:col-span-9 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Rua</label><input value={formData.settings?.address?.street} onChange={e => setAddressValue('street', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                            <div className="col-span-12 md:col-span-2 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Nº</label><input value={formData.settings?.address?.number} onChange={e => setAddressValue('number', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                            <div className="col-span-12 md:col-span-5 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Bairro</label><input value={formData.settings?.address?.neighborhood} onChange={e => setAddressValue('neighborhood', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                            <div className="col-span-12 md:col-span-5 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Complemento</label><input value={formData.settings?.address?.complement} onChange={e => setAddressValue('complement', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                            <div className="col-span-12 md:col-span-8 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Cidade</label><input value={formData.settings?.address?.city} onChange={e => setAddressValue('city', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm h-[46px]"/></div>
                            <div className="col-span-12 md:col-span-4 space-y-1.5"><label className="text-[10px] font-bold uppercase ml-1">Estado</label><input value={formData.settings?.address?.state} onChange={e => setAddressValue('state', e.target.value)} className="w-full bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm text-center h-[46px]" maxLength={2} /></div>
                        </div>
                    </div>
                  </div>
                  <div className="p-4 bg-slate-50 dark:bg-gray-800/50 flex justify-end gap-3"><button onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/5">Cancelar</button><button onClick={handleSaveClient} className="px-6 py-2 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-lg text-xs h-[46px]">Salvar</button></div>
                </div>
              </div>
            )}
            {generatedPasswordInfo && (<div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
                <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95">
                    ...
                    <div className="p-6 bg-slate-50 dark:bg-gray-800/50 flex flex-col sm:flex-row gap-3">
                        ...
                        <button onClick={handleCopyCredentials} className="flex-1 px-6 py-3 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl shadow-lg uppercase tracking-widest text-xs flex items-center justify-center gap-2 h-[46px]">{credentialsCopySuccess ? <><Check size={16}/> Copiado!</> : <><Copy size={14}/> Copiar Dados</>}</button>
                    </div>
                </div>
            </div>)}
